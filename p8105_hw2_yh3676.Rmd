...
```
* The dataset `amyloid_df` is imported by skipping the first row. Use `pivot_longer` to switch the dataset from “wide” to “long” format. The column name 'study_id' is changed to 'id' to match with the `baseline_df` dataset. The data type of `amyloid_ratio` is converted to numeric through `mutate`. 
* The dateset `amyloid_df` has `r nrow(amyloid_df)` observations and `r ncol(amyloid_df)` variables. The names of variables are: `r names(amyloid_df)`.
...
```
* The resulting dataset `combined_df` contains the data of `r combined_df |> pull(id) |> unique() |> length()` participants who appear in both datasets, including their basic demographic information (`id`,  `current_age`, `sex`, `education` ), APOE4 carrier status (`apoe4`), `age_at_onset` of MCI and their amyloid β 42/40 ratio (`amyloid_ratio`) at the time of measurement (`time`).

* `r combined_df |> filter(apoe4 == "carrier") |> pull(id) |> unique() |> length()` of the participants are APOE4 carriers, and `r combined_df |> filter(apoe4 == "carrier", age_at_onset > 0) |> pull(id) |> unique() |> length()` of them developed MCI. `r combined_df |> filter(apoe4 == "non-carrier") |> pull(id) |> unique() |> length()` of the participants are non-carriers, and `r combined_df |> filter(apoe4 == "non-carrier", age_at_onset > 0) |> pull(id) |> unique() |> length()` of them developed MCI.
