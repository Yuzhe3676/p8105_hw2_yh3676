...
* The dateset `amyloid_df` has `r nrow(amyloid_df)` observations and `r ncol(amyloid_df)` variables, with variale names: `r names(amyloid_df)`.
...
```
* The resulting dataset `combined_df` contains the data of `r combined_df |> pull(id) |> unique() |> length()` participants who appear in both datasets, including their basic demographic information (`id`,  `current_age`, `sex`, `education` ), APOE4 carrier status (`apoe4`), `age_at_onset` of MCI and their amyloid β 42/40 ratio (`amyloid_ratio`) at measurement time (`time`).
